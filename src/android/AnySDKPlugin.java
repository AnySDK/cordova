...
public class AnySDKPlugin extends CordovaPlugin {
    ...
    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
      ...
      if (info != null) {
        // appKey, appSecret, privateKey, oauthServer这些用户指定的参数将伴随插件的安装，被存贮在AndroidManifest.xml的<meta-data>标签里。
        // 在这里把它们读取出来，作为initPluginSystem()函数的参数。
          ...
          String privateKey = info.metaData.getString("com.anysdk.privatekey");
          String oauthServer = info.metaData.getString("com.anysdk.oauthserver");
          
          AnySDK.getInstance().initPluginSystem(cordovaActivity, appKey, appSecret, privateKey, oauthServer);
                ...
                switch(code)
                  {
                    ...
                    case UserWrapper.ACTION_RET_ANTIADDICTIONQUERY://防沉迷查询回调
                      fireJSEvent("anysdk.antiAddictionQuery", "anti-addiction query");
                      break;
                    case UserWrapper.ACTION_RET_REALNAMEREGISTER://实名注册回调
                      fireJSEvent("anysdk.realnameRegister", "realname register");
                      break;
                    case UserWrapper.ACTION_RET_ACCOUNTSWITCH_SUCCESS://切换账号成功回调
                      fireJSEvent("anysdk.accountSwitch.success", "account switch success");
                      break;
                    case UserWrapper.ACTION_RET_ACCOUNTSWITCH_FAIL://切换账号失败回调
                      fireJSEvent("anysdk.accountSwitch.fail", "account switch fail");
                      break;
              ...
              public void onCallBack(int arg0, String arg1) {
                switch(arg0)
                {
                  ...
                  case IAPWrapper.PAYRESULT_NETWORK_ERROR://支付超时回调
                    fireJSEvent("anysdk.payment.networkError", "payment timeout");
                    break;
                  case IAPWrapper.PAYRESULT_PRODUCTIONINFOR_INCOMPLETE://支付信息提供不完全回调
                    fireJSEvent("anysdk.payment.productInfoIncomplete", "production information incomplete");
                    break;
                    /**
                     * 新增加:正在进行中回调
                  ...
                  case IAPWrapper.PAYRESULT_NOW_PAYING:
                    fireJSEvent("anysdk.payment.pendingPayment", "pending payment");
                    break;
                  default:
                    break;
                }
              ...
              public void onCallBack(int arg0, String arg1) {
                  ...
                  switch (arg0) {
                    ...
                    case AdsWrapper.RESULT_CODE_PointsSpendFailed://积分墙消费失败
                      fireJSEvent("anysdk.ads.pointsSpendFailed", "points spend fail");
                      break;
                    case AdsWrapper.RESULT_CODE_PointsSpendSucceed://积分墙消费成功
                      fireJSEvent("anysdk.ads.pointsSpendSucceed", "points speed succeed");
                      break;
                    case AdsWrapper.RESULT_CODE_OfferWallOnPointsChanged://积分墙积分改变
                      fireJSEvent("anysdk.ads.offerWallPointsChanged", "offerwall points changed");
                      break;
              ...
              public void onCallBack(int arg0, String arg1) {
                  ...
                  switch (arg0) {
                    case PushWrapper.ACTION_RET_RECEIVEMESSAGE:
                      fireJSEvent("anysdk.push.receiveMessage", "receive message");
                      break;
                    default:
                      break;
                  }
            ...
            public void onCallBack(int arg0, String arg1) {
                ...
                switch (arg0) {
                  case SocialWrapper.SOCIAL_SIGNIN_FAIL://社交登陆失败
                    fireJSEvent("anysdk.social.signin.fail", "social signin fail");
                    break;
                  case SocialWrapper.SOCIAL_SIGNIN_SUCCEED://社交登陆成功
                    fireJSEvent("anysdk.social.signin.success", "social signin success");
                    break;
                  case SocialWrapper.SOCIAL_SIGNOUT_FAIL://社交登出失败
                    fireJSEvent("anysdk.social.signout.fail", "social signout fail");
                    break;
                  case SocialWrapper.SOCIAL_SIGNOUT_SUCCEED://社交登出成功
                    fireJSEvent("anysdk.social.signout.success", "social signout success");
                    break;
                  case SocialWrapper.SOCIAL_SUBMITSCORE_FAIL://提交分数失败
                    fireJSEvent("anysdk.social.submitScore.fail", "social submit score fail");
                    break;
                  case SocialWrapper.SOCIAL_SUBMITSCORE_SUCCEED://提交分数成功
                    fireJSEvent("anysdk.social.submitScore.success", "social submit score success");
                    break;
                  default:
                    break;
                }
      ...
      // 监听Activity的onStart, onStop, onPause, onResume事件，并在这些事件触发时，调用相应的AnySDK统计API
      this.activity.getApplication().registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks(){
        ...
        public void onActivityStarted(Activity activity) {
          ...
          // Hack here. Cordova plugin does not provide "onRestart()" handle.
    ...
    private void onChargeOnlySuccess(Map<String, String> paramMap) {
        AnySDKParam param = new AnySDKParam(paramMap);
        AnySDKAnalytics.getInstance().callFunction("onChargeOnlySuccess",param);
    }
    ...
    private void isAdTypeSupported(String type, CallbackContext callbackContext) {
        ...
        switch (type) {
            ...
            case "AD_TYPE_FULLSCREEN":
                ...
                break;
            case "AD_TYPE_MOREAPP":
                adType = AdsWrapper.AD_TYPE_MOREAPP;
                break;
            case "AD_TYPE_OFFERWALL":
    ...
    private boolean isAppOnForeground() {
        ActivityManager activityManager = (ActivityManager) this.activity.getApplicationContext().getSystemService(Context.ACTIVITY_SERVICE);
     ...
     */
    private Map<String, String> getMap(JSONArray args, int index) throws JSONException {
     ...
     */
    private ArrayList<String> getArrayList(JSONArray args, int index) throws JSONException {
